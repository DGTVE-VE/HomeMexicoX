<div id="fb-root"></div>
<ul class="col-lg-12 col-md-12 col-sm-12 col-xs-12 nav navbar-nav navbar-left lineaSeparacion"> 
    <li class="col-lg-1 col-md-1 col-sm-3 col-xs-6">
        <a href="http://mx.mexicox.gob.mx/contact" class="textoLigas"><strong>Contacto</strong></a>
    </li>
    <li class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
        <a href="http://mx.mexicox.gob.mx/donate" class="textoLigas"><strong>Servicio Social</strong></a>
    </li> 
    <li class="col-lg-1 col-md-1 col-sm-3 col-xs-6"><a href="http://mx.mexicox.gob.mx/faq" class="textoLigas"><strong>FAQs</strong></a></li>
    <li class="col-lg-1 col-md-1 col-sm-3 col-xs-6"><a href="http://mx.mexicox.gob.mx/about" class="textoLigas"><strong>Nosotros</strong></a></li>
    <li class="col-lg-2 col-md-3 col-sm-3 col-xs-6">
        <a href="{{asset('aliados')}}" class="textoLigas"><strong>Aliados Estratégicos</strong></a>
    </li>
    <li class="col-lg-2 col-md-1 col-sm-2 col-xs-6">
        <a href="{{asset('blog')}}" class="textoLigas"><strong>Blog</strong></a>
    </li>
    <li class="col-lg-3 col-md-3 col-sm-5 col-xs-12 margenRedesSoc">
        <!--FB. Me gusta-->
        <div class="fb-like" data-href="https://www.facebook.com/TvEducativaMx/" data-layout="button_count" data-action="like" data-size="small" data-show-faces="false" data-share="false"></div>
        <div class="fb-share-button" data-href="http://mexicox.gob.mx/" data-layout="button_count"></div>
    </li>
</ul>  
<div class="col-lg-1 col-md-2 col-sm-4 col-xs-6">
    <a href="http://www.gob.mx/sep">
        <img src="{{url('img/sep.png')}}" alt="Secretaría de Educación Pública" class="alturaImg">
    </a>
</div>
<div class="col-lg-2 col-md-2 col-sm-4 col-xs-6">
    <center>
        <a href="https://www.gob.mx/mexicodigital/">
            <img src="{{url('img/mexico_digital.png')}}" alt="México Digital" class="imgMexDig">
        </a>
    </center>
</div>
<div class="visible-xs col-xs-12 division"></div>
<div class="col-lg-1 col-md-2 col-sm-4 col-xs-6">
    <a href="http://www.televisioneducativa.gob.mx/">
        <img src="{{url('img/dgtve.png')}}" alt="Televisión Educativa" class="alturaImg">
    </a>
</div>
<div class="visible-sm col-sm-12 division"></div>
<div class="col-lg-1 col-md-2 col-sm-4 col-xs-6">
    <a href="http://mexicox.gob.mx/">
        <img src="{{url('img/mexicox.png')}}" alt="MéxicoX" class="imgMexicoX">
    </a>
</div>
<div class="visible-xs col-xs-12 division"></div>
<div class="hidden-md  hidden-sm hidden-xs col-lg-4"></div>
<div class="col-lg-2 col-md-2 col-sm-4 col-xs-6">
    <center>
        <a href="http://open.edx.org">
            <img src="https://files.edx.org/openedx-logos/edx-openedx-logo-tag.png" alt="Con la tecnología de Open edX" class="alturaImg">
        </a>
    </center>
</div>
<div class="col-lg-1 col-md-2 col-sm-4 col-xs-6">
    <a href="http://groups.itu.int/stocktaking/WSISPrizes/WSISPrizes2016.aspx#home">
        <img src="{{url('img/logo-swis.png')}}" alt="Premio WSIS" class="alturaImg">
    </a>
</div>
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <br>
    <p class="text-muted col-md-12 col-sm-12 txtLegales">© MéxicoX. Todos los derechos reservados, excepto cuando se indique. EdX, Open edX, y los logos de edX y Open edX son marcas registradas de edX Inc.</p>        
    <a href="http://mx.mexicox.gob.mx/privacy" class="colorLegales txtLegales" aria-expanded="false">Política de privacidad </a> <span style="padding-right:10px; padding-left:10px;"> - </span>
    <a href="http://mx.mexicox.gob.mx/honor" class="colorLegales txtLegales" aria-expanded="false">Código de Honor</a>

</div>
<style>
    .alturaImg{
        height: 40px;
    }
    .imgMexDig{
        height: 35px;
    }
    .imgMexicoX{
        height:48px; margin-top: -5px;
    }
    .textoLigas{
        color: black;
    }
    .division{
        padding:10px;
    }
    .txtLegales{
        font-size: 10px;
    }
    .colorLegales{
        color: red; 
    }
    .lineaSeparacion{
        border-top: solid 2px black;
    }
    .margenRedesSoc{
        padding-top:15px;
       @media(max-width:500px){
           padding-bottom:15px;
       }
    }
</style>
<div id="fb-root"></div>
<script>
    (function(d, s, id) {
        var js, fjs = d.getElementsByTagName(s)[0];
        if (d.getElementById(id)) return;
        js = d.createElement(s); js.id = id;
        js.src = "//connect.facebook.net/es_ES/sdk.js#xfbml=1&version=v2.9";
        fjs.parentNode.insertBefore(js, fjs);
    }(document, 'script', 'facebook-jssdk'));
</script>